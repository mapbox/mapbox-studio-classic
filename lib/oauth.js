...
app.use('/oauth/config', function(req, res, next){
    ...
    tm._config.MapboxTile = req.query.MapboxTile || MapboxTile;

    if (tm._config.MapboxTile === MapboxTile) return res.redirect('/oauth/mapbox');
...
app.use('/oauth/atlas', function(req, res, next){
    if (tm._config.MapboxTile === 'http://a.tiles.mapbox.com') return res.redirect('/oauth/mapbox');
    var MapboxTile = tm._config.MapboxTile || 'http://localhost:2999';
    http.get(MapboxTile + '/atlas/config', function(response){
        ...
        response.on('data', function(data){
            if (data.replace(/^\s+|\s+$/g, '') == 'Not Found'){
                ...
                tm._config.MapboxTile = MapboxTile,
                res.redirect('/authorize');
            }
        });
    }).on('error', function(e){
