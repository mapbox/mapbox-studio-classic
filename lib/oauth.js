...
app.use('/oauth/config', function(req, res, next){
    var MapboxAPITile = 'https://a.tiles.mapbox.com/';

    if (/(?:http:\/\/)?a\.tiles\.mapbox\.com\/?/.test(req.query.MapboxAPITile)) req.query.MapboxAPITile = MapboxAPITile;
    tm.db.set('MapboxAPITile', req.query.MapboxAPITile || MapboxAPITile);

    if (tm.apiConfig('MapboxAPITile') === MapboxAPITile) return res.redirect('/oauth/mapbox');
...
app.use('/oauth/atlas', function(req, res, next){
    ...
    http.get(MapboxAPITile + '/atlas/config', function(response){
