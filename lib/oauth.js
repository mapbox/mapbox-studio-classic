...
app.use('/oauth/atlas', function(req, res, next){
    ...
    fs.readFile(process.env.HOME + '/.mapbox-studio/atlasEnv.json', {encoding: 'utf8'}, function(err, params){
        params = JSON.parse(params) || { location: 'http://localhost:2999' };
        http.get(params.location + '/atlas/config', function(response){
            ...
            response.on('data', function(data){
                if (data.replace(/^\s+|\s+$/g, '') == 'Not Found'){
                    ...
                    tm._config.MapboxTile = JSON.parse(data).tileHost,
                    res.redirect('/authorize');
                }
            });
        }).on('error', function(e){
