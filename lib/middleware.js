...
middleware.userTilesets = function(req, res, next) {
    ...
    var url;
    if (tm.oauth().isMapboxAPI) url = tm.apiConfig('MapboxAPIAuth')+'/api/Map?account='+oauth.account+'&_type=tileset&private=true&access_token='+oauth.accesstoken;
    else url = tm.apiConfig('MapboxAPITile')+'/atlas/maps';
...
middleware.newStyle = function(req, res, next) {
    ...
    if (req.query && req.query.source) {
        ...
        var rid = (req.query && req.query.id) || style.tmpid();
        style.clear(rid);
        style(rid, function(err, s) {
            if (err) return next(err);
            req.style = s;
            next();
        });
...
middleware.offlineConfig = function(req, res) {
    tm.db.set('oauth', {
        account: 'offline',
        accesstoken: '',
        isMapboxAPI: false
