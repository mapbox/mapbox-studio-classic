...
middleware.normalizePaths = function(req, res, next) {
    ['id','source','style','saveas'].forEach(function(key) {
...
middleware.saveMBTiles = function(req, res, next) {
    if (req.query.saveas) {
        try {
            var dest = fs.createWriteStream(req.query.saveas);
        } catch(err) {
            return next(err);
        }
        source.toMBTiles(req.source.data.id, dest, function(err) {
            if (err) next(err);
            res.send(204);
        });
    } else {
