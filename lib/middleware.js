...
middleware.loadStyle = function(req, res, next) {
    style(req.query.id, function(err, s) {
        ...
        if (err) return next(err);
        if (!tm.tmpid('tmstyle:', s.data.id)) tm.history('style', s.data.id);
        var watchpath = s.data.id.replace('tmstyle://', '')+'/*.mss';
        if(!watching[watchpath]){

            gaze(watchpath, function(err, watcher) {
                // On changed/added/deleted
                watching[watchpath] = this;
                this.on('all', function(event, filepath) {
                    style(req.query.id, {forceReload:true}, function(err, s) {
                        if(err) return;
                        style.toXML(s.data, function(err, xml){
                            style.refresh(s.data, xml, function(err){
                                middleware.events.emit('message', {});
                            });
                        });
    ...
    request(tm._config.mapboxauth+'/api/Map/'+tm.db._docs.oauth.account+'.tm2-basemap?access_token='+tm.db._docs.oauth.accesstoken, function(error, response, body) {
        ...
        if (response.statusCode >= 400) {
            ...
            request({
                ...
                if (response.statusCode === 401) return res.redirect('/unauthorize');
