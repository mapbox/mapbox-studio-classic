...
middleware.userTilesets = function(req, res, next) {
    ...
    var url = tm.apiConfig('MapboxAPIAuth')+'/api/Map?account='+oauth.account+'&_type=tileset&private=true&access_token='+oauth.accesstoken;
...
middleware.config = function(req, res, next) {
    if (/(?:http:\/\/)?a\.tiles\.mapbox\.com\/?/.test(req.query.MapboxAPITile)) req.query.MapboxAPITile = 'https://a.tiles.mapbox.com/';
    tm.db.set('MapboxAPITile', req.query.MapboxAPITile || 'https://a.tiles.mapbox.com/');

    request.get(tm.apiConfig('MapboxAPITile'), function(err, response, body){
        if (err) return next(new Error('Cannot find Mapbox API at ' + tm.apiConfig('MapboxAPITile')));

        var api = JSON.parse(body).api;
        if (api === 'mapbox') return res.redirect('/oauth/mapbox');
        else if (api === 'atlas') return res.redirect('/oauth/atlas');
        else return next(new Error('Cannot find Mapbox API at ' + tm.apiConfig('MapboxAPITile')));
    });
};

middleware.atlasConfig = function(req, res, next) {
    if (/http:\/\/a\.tiles\.mapbox\.com\/?/.test(tm.apiConfig('MapboxAPITile'))) return res.redirect('/oauth/mapbox');
    var MapboxAPITile = tm.apiConfig('MapboxAPITile') || 'http://localhost:2999';
