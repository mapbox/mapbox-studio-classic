...
var mapnikReference = require('mapnik-reference');
...
app.get('/source', middleware.source, middleware.history, middleware.examples, function(req, res, next) {
    ...
    try {
        var page = tm.templates.source({
			...
			sourceRef: mapnikReference.load('3.0.0').datasources
        });
    } catch(err) {
...
app.get('/app/sourceref.js', function(req, res, next) {
    ...
    res.send('window.sourceRef = ' + JSON.stringify(require('mapnik-reference').load('3.0.0').datasources) + ';');
});
