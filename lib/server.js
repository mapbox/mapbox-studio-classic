...
var sourceDocs = require('../app/datasources.json').datasources;
...
app.get('/style', middleware.style, middleware.history, middleware.examples, function(req, res, next) {
    ...
    try {
        var page = tm.templates.style({
            ...
            version: version,
            mapboxAPI: { isMapboxAPI: tm.oauth().isMapboxAPI, MapboxAPITile: tm.apiConfig('MapboxAPITile') },
            oauth: tm.oauth()
        });
    } catch(err) {
...
app.get('/source', middleware.source, middleware.history, middleware.examples, function(req, res, next) {
    ...
    try {
        var page = tm.templates.source({
            ...
            saferstringify: saferstringify,
            version: version,
            isMapboxAPI: tm.oauth().isMapboxAPI,
            sourceRef: sourceDocs
        });
    } catch(err) {
...
app.get('/font.png', function(req, res, next) {
    ...
    // Get font directory, only for user fonts
    if (req.query.style) {
        style(req.query.style, function(err, s) {
            if (err) return next(err);
            var fontdir = tm.join(tm.parse(req.query.style).pathname, s._map.fontDirectory());
            tm.font(req.query.id, req.query.text||'', fontdir, send);
        });
    } else {
        tm.font(req.query.id, req.query.text||'', undefined, send);
    }
...
app.get('/app/sourceref.js', function(req, res, next) {
    res.set({
        'content-type':'application/javascript',
        'cache-control':'max-age=3600'
    });
    res.send('window.sourceRef = ' + JSON.stringify(sourceDocs) + ';');
});
...
app.delete('/history/:type(style|source)', function(req, res, next) {
    if (!req.query.id) return next(new Error('No id specified'));
...
app.get('/geocode', middleware.userTilesets, function(req, res, next) {
    var endpoint = tm.oauth().isMapboxAPI ? 'http://api.tiles.mapbox.com' : tm.apiConfig('MapboxAPITile');
    var query = endpoint + '/v4/geocode/mapbox.places/{query}.json?access_token=' + req.accesstoken;
    res.redirect(query.replace('{query}', req.query.search));
});

//Calls mapnik-omnivore for file's metadata, or mapnik for extent
app.get('/metadata', function (req, res, next) {
    ...
    if (req.query.file) {
        ...
        console.log('server, req.query.ds:', req.query.ds);
        console.log('server, req.headers.referer:', req.headers.referer);
        var layer = req.query.ds;
        var tmsrc = req.headers.referer.split("=")[1];
        source.info(tmsrc, function (err, data) {
            console.log('server info, err:', err);
            //console.log('server info, data:', JSON.stringify(data));
            if (err) return next(err);
            data.Layer = _.filter(data.Layer, function (lyr) { return lyr.id === layer; });
            if (data.Layer[0].Datasource.table) {
                if (!/scale_denominator/i.test(data.Layer[0].Datasource.table)) {
                    data.Layer[0].Datasource.extent_from_subquery = true;
                }
            }
            var ext = source.extent(data)
            console.log('server info, extent:', ext);
            res.send({ extent: ext });
        });
    }
});
