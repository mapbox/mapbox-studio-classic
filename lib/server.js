...
app.get('/config', function(req, res, next) {
    fs.readFile(path.join(process.env.HOME, '.mapbox-studio', 'atlasEnv.json'), {encoding: 'utf8'}, function(err, params){
        params = (err) ? { MapboxTile: 'http://locahost:2999' } : JSON.parse(params);
        res.send(tm.templates.config({
            MapboxTile: params.MapboxTile,
            error: false
        }));
    });
});
...
app.get('/style', middleware.style, middleware.history, middleware.examples, function(req, res, next) {
    ...
    try {
        var page = tm.templates.style({
            ...
            version: version,
            MapboxTile: tm._config.MapboxTile
        });
    } catch(err) {
...
app.get('/config/save', function(req, res, next) {
    // save to environment variables
    var redirect = req.query.redirect || null;
    delete req.query.redirect;

    // write parameters to disk so they can be accessed on start up
    fs.writeFile(path.join(process.env.HOME, '.mapbox-studio', 'atlasEnv.json'),  JSON.stringify(req.query), function(){});
    tm._config.MapboxTile = req.query.MapboxTile;

    if (redirect) res.redirect(redirect);
    else res.send(tm.db.get('user'));
});
