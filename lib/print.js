...
function print(arg, callback) {
    ...
    // Reading.
    print.info(id, function(err, data) {
        if (err) return callback(err);
        print.toXML(data, function(err, xml) {
            if (err) return callback(err);
            print.refresh(data, xml, callback);
        });
    });
};
...
//         var suri = tm.parse(filtered.source);
//         filtered.source = suri.protocol === 'tmsource:' && suri.dirname === uri.dirname ? '.' : filtered.source;
//         files.push({
...
// Helper for tm.tmpid().
print.tmpid = function(id, md5) { return tm.tmpid('tmstyle:', id, md5) };
...
print.info = function(id, callback) {
    var uri = tm.parse(id);
...
// Read style thumb.
print.thumb = function(id, callback) {
...
print.stats = function(id, key, z, val) {
