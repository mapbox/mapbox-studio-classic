...
tm.config = function(opts, callback) {
    ...
    mapnik.register_fonts(path.dirname(require.resolve('tm2-private-fonts')), { recurse: true });
...
tm.config({
    ...
    mapboxauth: 'https://tiles.mapbox.com',
    mapboxtile: 'http://a.tiles.mapbox.com/v3'
});
...
tm.remote = function(id, data) {
    var remote = tm.db.get('remote') || {};
    if (!data) {
        return remote[id] || false;
    } else {
        remote[id] = data;
        tm.db.set('remote', remote);
        return remote[id];
    }
};
        ...
        source(from, function(err, s) {
            ...
            s.getInfo(function(err, info) {
                ...
                var opts = {
                    ...
                    bbox: s.data.bounds || source.extent(s.data)
                };
