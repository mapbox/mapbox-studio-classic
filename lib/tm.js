...
tm.tmpid = function(protocol, id, md5) {
    if (id && !md5) return (new RegExp(protocol + '\/\/\/tmp')).test(id);
...
tm.parse = function(str) {
    ...
    if (uri.pathname) {
        uri.dirname = tm.join(unescape(uri.pathname));
        if (uri.host && uri.host.length === 1) uri.dirname = uri.host.toLowerCase() + ':' + uri.dirname;
    }
    return uri;
};
