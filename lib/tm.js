...
tm.apiConfig = function(api) {
    if (api === 'MapboxTile'){
        var MapboxTile = tm.db.get('MapboxTile');
        if (!MapboxTile) {
            tm.db.set('MapboxTile', 'https://a.tiles.mapbox.com/');
        }
        return MapboxTile;
    } else if (api === 'MapboxAuth'){
        var MapboxAuth = tm.db.get('MapboxAuth');
        if (!MapboxAuth) {
            tm.db.set('MapboxAuth', 'https://api.mapbox.com');
        }
        return MapboxAuth;
    }
};
