...
tm.project = function(options, callback) {
    ...
    var load = function(data, xml) {
        ...
        var done = function(err, project) {
            data.id = id;
            tm.cacheProject[id] = tm.cacheProject[id] || project;
            tm.cacheProject[id].data = data;
            tm.cacheProject[id].data.background = _('rgba(<%=r%>,<%=g%>,<%=b%>,<%=(a/255).toFixed(2)%>)').template(tm.cacheProject[id]._map.background);
            return tm.project(_(options).extend({data:null}), callback);
        };
        return tm.cacheProject[id]
            ? tm.cacheProject[id].update(opts, done)
