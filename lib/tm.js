...
tm.oauth = function() {
    ...
    oauth.isMapboxAPI = (oauth.isMapboxAPI === false) ? false : true;
    return oauth;
};

tm.apiConfig = function(api) {
    if (api === 'MapboxAPITile'){
        var MapboxAPITile = process.env.MapboxAPITile || tm.db.get('MapboxAPITile');
        if (!MapboxAPITile) {
            tm.db.set('MapboxAPITile', 'https://a.tiles.mapbox.com');
            MapboxAPITile = 'https://a.tiles.mapbox.com';
        }
        return MapboxAPITile;
    } else if (api === 'MapboxAPIAuth'){
        var MapboxAPIAuth = process.env.MapboxAPIAuth || tm.db.get('MapboxAPIAuth');
        if (!MapboxAPIAuth) {
            tm.db.set('MapboxAPIAuth', 'https://api.mapbox.com');
            MapboxAPIAuth = 'https://api.mapbox.com';
        }
        return MapboxAPIAuth;
    }
};
...
tm.srsname = {};
Object.keys(tm.srs).forEach(function(name) {
    tm.srsname[tm.srs[name]] = name;
});
...
tm.fontfamilies = function() {
    ...
    for (var i = 0; i < fonts.length; i++) {
        ...
        var family_joined = parts.join(' ');
        level1[family_joined] = level1[family_joined] || [];
        level1[family_joined].push(fonts[i]);
    }
    var level2 = {};
    for (var fam in level1) {
        if (level1[fam].length > 1) continue;

        var parts1 = fam.split(' ');
        if (parts1.length === 1) continue;
        parts1.pop();
        var family1 = parts1.join(' ');

        level2[family1] = level2[family1] || [];
        level2[family1].push(level1[fam][0]);
    }
    for (var fam2 in level1) {
        if (level1[fam2].length > 1) continue;

        var parts2 = fam2.split(' ');
        if (parts2.length === 1) continue;
        parts2.pop();
        var family2 = parts2.join(' ');

        if (level2[family2].length > 1) {
            delete level1[fam2];
            level1[family2] = level2[family2];
        }
    }
    for (var k in level1) {
    ...
    function famsort(a, b) {
        ...
        for (var j = 0; j < bparts.length; j++) {
            bindex = keywords.indexOf(bparts[j].toLowerCase());
...
tm.parse = function(str) {
    ...
    if (uri.pathname) {
        uri.dirname = unescape(uri.pathname);  // jshint ignore:line
