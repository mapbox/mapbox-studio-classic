    ...
    fs.stat(turi.pathname, function(err, stat) {
        ...
        source(from, function(err, s) {
            ...
            s.getInfo(function(err, info) {
                ...
                var tmp = 'mbtiles://' + path.join(tm.config().tmp, path.basename(turi.pathname));
                var job = path.join(tm.config().tmp, path.basename(turi.pathname)) + '.job';
                var opts = {
                ...
                var task = new tilelive.CopyTask(from, tmp, scheme, job);
                tm._copy = {
                    id: from,
                    file: null,
                    stat: null,
                ...
                tm._copy.task.on('finished', function(err) {
                    ...
                    function cleanup() {
                        ...
                        try {
                            fs.renameSync(url.parse(tmp).pathname, turi.pathname);
                        } catch (err) {
                            if (err && err.code === 'EBUSY') return setTimeout(cleanup, 1000);
