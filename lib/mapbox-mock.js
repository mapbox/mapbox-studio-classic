...
var creds = require('../node_modules/mapbox-upload/test/test.creds.js');
...
module.exports = function(app) {
    ...
    app.get('/badjson/api/upload/test', function(req, res, next){
        if (req.query.access_token = 'validtoken') {
            res.writeHead(200);
            res.end("hello world");
        }
    });
    app.get('/nokey/api/upload/test', function(req, res, next){
        if (req.query.access_token === 'validtoken') {
             res.writeHead(200);
            res.end(JSON.stringify({bucket:'bar'}));
        }
    });
    app.get('/nobucket/api/upload/test', function(req, res, next){
        if (req.query.access_token === 'validtoken') {
            res.writeHead(200);
            res.end(JSON.stringify({key:'bar'}));
        }
    });
    app.get('/api/upload/test', function(req, res, next){
        if (req.query.access_token === 'validtoken') {
            res.writeHead(200);
            res.end(JSON.stringify(creds));
        }
    });
    app.all('/api/Map/test*', function(req, res, next){
        if (req.query.access_token === 'validtoken') {
            if (req.method === 'GET') {
                res.writeHead(404);
