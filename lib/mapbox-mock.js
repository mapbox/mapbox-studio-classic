...
module.exports.AtlasServer = function(app) {
    app.use(app.router);
    app.get('/atlas/config', function(req, res, next) {
        res.send(
            {
              "AtlasServerPort": "2999",
              "tileHost": "http://localhost:2999",
            }
        );
    });

    app.get('/badurl/atlas/config', function(req, res, next) {
        res.writeHead(404);
        res.end('Not Found');
    });

    app.get('/badlocation/atlas/config', function(req, res, next) {
        return res.send(404);
    });
    return app;
};
