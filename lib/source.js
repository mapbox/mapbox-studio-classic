...
function source(arg, callback) {
    ...
    source.info(id, function(err, data) {
        ...
        source.toXML(data, function(err, xml) {
            if (err) return callback(err);
            source.refresh(data, xml, false, callback);
        });
    });
}

// Load or refresh the relevant source using specified data + xml.
source.refresh = function(data, xml, invalidate, callback) {
    var id = data.id;
    ...
    function done(err, p) {
        ...
        xray({
            ...
            return invalidate ? wipe() : callback(null, cache[id]);
        });
    }
...
source.save = function(data, callback) {
    ...
    if (remote) return source.info(id, function(err, info) {
        ...
        source.refresh(info, null, false, callback);
    });
    ...
    source.toXML(data, function(err, xml) {
        if (err) return callback(err);
        if (!perm) return source.refresh(data, xml, true, callback);
        ...
        tm.writefiles(uri.dirname, files, function(err) {
            if (err) return callback(err);
            data._tmp = false;
            source.refresh(data, xml, true, function(err, p) {
                if (err) return callback(err);
