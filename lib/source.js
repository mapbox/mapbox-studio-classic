...
source.upload = function(data, force, callback) {
    data.mapbox = data.mapbox ? data.mapbox : 'https://tiles.mapbox.com';
    ...
    function startUpload(file, info) {
        ...
        if (!data.mapid && info._prefs.dirty === false) return callback(null, new task.Done(data.id, 'upload', '/mbtiles?id=' + data.id, fs.statSync(file).size, info._prefs.mapid));
        // if the source has be re-exported, update flag is tripped
        info._prefs.mapid = info._prefs.mapid || setMapid(data, info);
        if (info._prefs.dirty) data.mapid = info._prefs.mapid;
        ...
        // other tasks to sneak in!
        if (force) task.del();
        if (task.get() && task.get().id === data.id) {
            return callback(null, task.get());
        }
        if (task.get()) return callback(new Error('Active task in progress'));

        task.set(source.mbtilesUpload(data, info, file, callback));
        callback(null, task.get());
    }
...
source.checkMapid = function(mapid, callback){
    ...
    var str = '';
    https.get('https://www.mapbox.com/api/Map/' + mapid, function(res, body){
...
source.mbtilesUpload = function(data, info, file, callback){
    var prog = upload({
        file: file,
    ...
    prog.once('finished', function(){ dataSaved(info, prog, callback); });

    return new task.Task(data.id, 'upload', prog);

    function dataSaved(info, prog, callback) {
        // save mapid to data.yml
        if (info._prefs.dirty === false) info._prefs.mapid = data.mapid;
        info._prefs.dirty = false;
        source.save(info, function(){
