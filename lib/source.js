...
source.upload = function(data, force, callback) {
    ...
    source.info(data.id, function(err, info) {
        source.toHash(data.id, function(err, hash){
            if (force) {
                data.mapid = setMapid(data, info, hash);
                startUpload(info);
            } else {
    ...
    function startUpload(info, hash) {
        // Export exists.
        if (!data.mapid && hash === info._prefs.rev) return callback(null, new task.Done(data.id, 'upload', '/mbtiles?id=' + data.id, 1000, info._prefs.mapid));
        data.mapid = data.mapid || info._prefs.mapid;
        // Check before setting creating task that there is no
        // active task. Remaining calls beyond this point are sync
        // ensuring that our task will be set without any gaps for
        // other tasks to sneak in!
...
source.uploadStream = function(data, info, callback) {
    ...
    function copy() {
        ...
        source.createUpload(data, info, prog, callback);
        read.on('finished', finish);
    }
...
source.createUpload = function(data, info, prog, callback){
    ...
    uploadProg.once('finished', function(){ dataSaved(info, data, uploadProg, callback); });

    function dataSaved(info, data, uploadProg, callback) {
        info._prefs.mapid = data.mapid;
        source.toHash(data.id, function(err, hash){
            info._prefs.rev = hash;
