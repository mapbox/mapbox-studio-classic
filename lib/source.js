...
var source = function(options, callback) {
    ...
    var load = function(data, xml) {
        var done = function(err, p) {
            ...
            project = project || require('./project');
            project({
                id: project.tmpid(id, true),
                data: {
                ...
                if (perm) project.thumb({
                    id: project.tmpid(id, true),
                    thumbpath: path.join(uri.pathname,'.thumb.png'),
                    perm: true
                });
                cache[id].project = source;
                cache[id].data._project = project.tmpid(id, true);
