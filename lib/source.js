...
source.refresh = function(rawdata, callback) {
    ...
    if (remote) {
        source.info(id, function(err, info) {
            ...
            var Ctj = new CachingTileJSON(TileJSON, tm.config().cache);
    ...
    // local sources
        ...
        source.toXML(data, function(err, x) {
            ...
            var Cb = new CachingBridge(Bridge, tm.config().cache);
...
source.uploadStream = function(id, mapid, info) {
    ...
    function copy() {
        ...
        uploadprog.once('error', function(err) { console.log(err); prog.emit('error', err); });
        uploadprog.once('finished', finish);
    }
