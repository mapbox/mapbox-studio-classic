...
source.mbtiles = function(id, force, callback) {
    ...
    function startExport(err, stat) {
        ...
        task.set(source.mbtilesExport(id, up, callback));
        callback(null, task.get());
    }
};

source.mbtilesExport = function(id, callback) {
    var tmp;
    var hash;
    var fsrc;
    var tsrc;
...
source.mbtilesStream = function(data, force, callback) {
    ...
    function startStream(info) {
        console.log('check', !data.mapid, dirty === false)
        // Export exists.
        // NOTE: this will not pass until mapbox-upload.putMap() completes
        if (!data.mapid && dirty === false) return callback(null, new task.Done(data.id, 'upload', '/mbtiles?id=' + data.id, 1000, info._prefs.mapid));
        ...
        task.set(source.mbtilesExportUpload(data, info, callback));
        callback(null, task.get());
    }
...
source.mbtilesExportUpload = function(data, info, callback) {
    var fsrc;
    var prog = progress({
        objectMode: true,
        time: 100
    ...
    function copy() {
        ...
        // prog.on('progress', function(d){console.log('SOURCE',d)})
        data.stream = read.pipe(tilelive.serialize()).pipe(prog).pipe(zlib.Gzip());
        source.mbtilesUpload(data, info, callback);
        // read.on('end', finish)
    }
...
source.mbtilesUpload = function(data, info, callback){
