...
function source(arg, callback) {
    ...
    if (protocolIsValid(uri.protocol) === 'local') {
        var id = uri.protocol + '//' + uri.dirname;
    } else if (protocolIsValid(uri.protocol) === 'remote') {
...
source.tmpid = function(id) {
    return id ? id.indexOf('tmpsource:') === 0 : 'tmpsource://' + tm.join(path.dirname(require.resolve('mapbox-studio-default-source')));
};
...
source.extent = function(data) {
    var default_extent = [-180,-85.0511,180,85.0511];
    var extent = (data.Layer||[]).reduce(function(memo, l) {
        ...
        try {
            ...
            ds_extent = getExtent(l.srs, ds.extent());
        } catch (err) {
            console.log(util.format('Could not load layer "%s" in datasource \'%s"\': "%s"', l.id, data.name, err));
            return default_extent;
        }

        if (!ds_extent) {
            console.log(util.format('Missing extent for layer "%s", defaulting to "%s"', l.id, default_extent.join(',')));
            return default_extent;
        } else {
            // Take the greater coordinate to make sure all layers are included in the final extent
            memo[0] = Math.max(-180, Math.min(ds_extent[0], memo[0]));
            memo[1] = Math.max(-85.0511, Math.min(ds_extent[1], memo[1]));
            memo[2] = Math.min(180, Math.max(ds_extent[2], memo[2]));
            memo[3] = Math.min(85.0511, Math.max(ds_extent[3], memo[3]));
            return memo;
        }
    }, [Infinity,Infinity,-Infinity,-Infinity]);

    // defaults to global extent if no other extent is provided
    return extent[0] === Infinity ? default_extent : extent;

    // reproject extent to WGS84 if necessary
    function getExtent(projection, ds_extent) {
        console.log('ds_extent:', ds_extent);
        ...
        if (current !== wgs84) {
            ...
            extent = transform.forward(ds_extent);
            console.log('extent:', extent);
            return extent;
        } else return ds_extent;
    };
};
...
source.normalize = function(data) {
    ...
    data.Layer = data.Layer.map(function(l) {
        ...
        var spec = datasources_spec[l.Datasource.type];
    ...
    // will be set to [], instead of undefined. And this logic will succeed: https://github.com/mapbox/mapbox-studio-classic/blob/raster_local_fix/lib/style.js#L216
...
source.info = function(id, callback) {
    ...
    var loadRemote = function () {
        ...
        if (uri.protocol === 'mapbox:') {
            ...
            url = tm.apiConfig('MapboxAPITile') + '/v4' + uri.pathname + '.json?secure=1&access_token='+oauth.accesstoken;
        } else {
            url = id;
        }
        ...
        new TileJSON(url, function(err, tj){
            if (err) return callback(err);
            try { data = tj.data; }
...
source.upload = function(id, force, callback) {
    ...
    function startUpload(info, hash) {
        ...
        task.set(source.uploadStream(id, mapid, info));
        callback(null, task.get());
    }
};

source.uploadStream = function(id, mapid, info) {
    var fsrc;
    var prog = progress({
        objectMode: true,
        time: 100
    ...
    function copy() {
        ...
        var uploadprog = upload({
            ...
            mapid: mapid,
            mapbox: tm.apiConfig('MapboxAPIAuth')
        })
        .once('error', function(err) { console.log(err); prog.emit('error', err); })
