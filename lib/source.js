...
source.upload = function(data, force, callback) {
    ...
    function startUpload(info, hash) {
        ...
        if (!force && hash === info._prefs.rev) {
            return callback(null, new task.Done(data.id, 'upload', '/mbtiles?id=' + data.id, 1000, info._prefs.mapid));
        }

        // Use the existin mapid or create a new one.
        data.mapid = info._prefs.mapid || data.oauth.account + '.' + crypto.createHash('md5')
