...
source.mbtiles = function(id, up, force, callback) {
...
source.mbtilesExport = function(id, up, callback) {
    var tmp;
    var hash;
    var fsrc;
    var tsrc;
    ...
    function copy() {
        ...
        if (up) {
            source.upload({
                stream: read.pipe(tilelive.serialize()).pipe(prog),
                id: id,
...
source.upload = function(data, force, callback) {
    ...
    function startUpload(file, info) {
        ...
        if (!data.mapid && dirty === false) return callback(null, new task.Done(data.id, 'upload', '/mbtiles?id=' + data.id, fs.statSync(file).size, info._prefs.mapid));
        ...
        source.mbtilesUpload(data, info, file, callback)
        callback(null, task.get());
    }
