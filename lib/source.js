...
source.upload = function(data, force, callback) {
    source.info(data.id, function(err, info) {
        source.toHash(info.id, function(err, file) {
            ...
            //     data.mapid = null;
            // } else {
            data.mapid = (typeof info._prefs.mapid !== 'undefined' && info._prefs.mapid !== '')
                ? info._prefs.mapid
    ...
    function startUpload(file) {
        ...
        // Upload complete.
        // finished is a temporary state holder until there are checks for mapids
        if (finished === true) return callback(null, new task.Done(data.id, 'upload', '/mbtiles?id=' + data.id, 1000));
        ...
        task.set(source.mbtilesUpload(data, file, callback));
        console.log('starting');
        callback(null, task.get());
    }
};

source.mbtilesUpload = function(data, file, callback){
    var prog = upload({
        file: file,
    ...
    prog.once('finished', function(){ dataSaved(data, callback) });

    return new task.Task(data.id, 'upload', prog);

    function dataSaved(data, callback) {
        console.log('finished')
        finished = true;
        // return callback(null, new task.Done(data.id, 'upload', '/mbtiles?id=' + data.id, 1000));
    };
