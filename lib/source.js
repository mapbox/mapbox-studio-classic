...
source.mbtiles = function(id, force, callback) {
    ...
    function startExport(err, stat) {
        ...
        source(id, function(err, s) {
            ...
            task.set(source.mbtilesExport(s));
...
source.mbtilesExport = function(fsrc) {
    var tmp;
    var hash;
    var tsrc;
    ...
    // Return task object immediately.
    return new task.Task(fsrc.data.id, 'export', read, prog);

    function loadHash() {
        source.toHash(fsrc.data.id, function(err, file) {
...
source.upload = function(data, callback) {
