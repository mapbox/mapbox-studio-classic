...
source.extent = function(data) {
    var extent = (data.Layer||[]).reduce(function(memo, l) {
        ...
        if (opts.type === 'shape' || opts.type === 'csv' || opts.type === 'gdal' || opts.type === 'postgis') {
            // Convert datasource extent to lon/lat when saving
            var fromProj = new mapnik.Projection(l.srs);
        ...
        } else if (opts.type === 'ogr') {
            try {
                extent = ds.extent();
            } catch (err) {
                return invalid('Error obtaining extent of ogr Mapnik datasource.');
            }
        }
