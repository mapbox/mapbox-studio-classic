...
source.toXML = function(data, callback) {
    ...
    // If local raster source/image file
    if (data.format === 'webp'){
        opts.Stylesheet = [{
            id: 'rasters.mss',
            data: '#_image { raster-opacity:1; raster-scaling:bilinear; }'
        }];
        opts.json = JSON.stringify(opts.Layer[0]);
    } else {
        // If there are composited raster layers present add a default style for each.
...
source.normalize = function(data) {
    ...
    if(local && data.Layer[0].Datasource.type === 'gdal') data.format === 'webp';
