...
source.extent = function(data) {
    var extent = (data.Layer||[]).reduce(function(memo, l) {
        ...
        // Ugh. Also, Windows.
        if (opts.file && opts.file.charAt(0) !== '/') opts.base = tm.parse(data.id).dirname;
...
source.info = function(id, callback) {
    ...
    var loadRemote = function () {
        var loaded = false,
            url = uri.protocol === 'mapbox:' ?  tm.config().mapboxtile + uri.pathname + '.json?access_token='+tm.db._docs.oauth.accesstoken : id;
        if (tm.remote(id)) {
