...
source.normalize = function(data) {
    ...
    data.vector_layers = tm.parse(data.id).protocol === 'tmsource:'
        ? data.Layer.reduce(function(vlayers, l) {
            // Only allow one vector_layer for raster sources
            if (l.Datasource.type === 'gdal' && vlayers.length > 0) return vlayers;

            var info = {};

            // For rasters, set id to _image
            info.id = l.Datasource.type === 'gdal' ? '_image': l.id;
