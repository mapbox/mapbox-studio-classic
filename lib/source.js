...
source.toXML = function(data, callback) {
    ...
    // Include map parameter list of layer IDs.
    opts.layers = _(opts.Layer).pluck('id').join(',');
...
source.info = function(id, callback) {
    ...
    switch (uri.protocol) {
    case 'mapbox:':
        fs.readFile(path.join(__dirname + '/../sources',uri.pathname,'data.yml'), 'utf8', function(err, data) {
            if (err) return callback(err);
