...
source.mbtilesStream = function(id, force, callback) {
    console.log('here')
    startStream();
    ...
    function startStream() {
        ...
        task.set(source.mbtilesExportUpload(id, callback));
        callback(null, task.get());
    }
};

source.mbtilesExportUpload = function(id, callback) {
    var fsrc;
    var prog = progress({
        objectMode: true,
        time: 100
    ...
    function copy() {
        ...
        source.upload({
            stream: read.pipe(tilelive.serialize()).pipe(prog).pipe(zlib.Gzip()),
            // prog: prog,
            id: id,
            oauth: tm.db.get('oauth')
        }, false, callback);
...
source.mbtilesUpload = function(data, info, callback){
    ...
    uploadProg.on('progress', function(d){console.log('upload', d)})
