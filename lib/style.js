...
style.save = function(data, callback) {
    ...
    if (perm && data._tmp) return tm.copydir(tm.parse(data._tmp).dirname, uri.dirname, function(err) {
        if (err) return callback(err);
        data._tmp = false;
...
style.tmpid = function(id) {
    return id ? id.indexOf('tmpstyle:') === 0 : 'tmpstyle://' + path.dirname(require.resolve('mapbox-studio-default-style'));
};
...
style.toPackage = function(id, dest, callback) {
    ...
    style(id, function(err, source) {
        if (err) return callback(err);
        if (source._source.split(':')[0] === 'tmsource') {
            return callback(new Error('Cannot package a local source with a style.'));
        }
