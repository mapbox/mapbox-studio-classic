...
style.save = function(data, callback) {
    ...
    if (data.layers) {
        style.layervalidate(data.layers, function(err) {
            if (err) return callback(err);
        });
    }
...
style.layervalidate = function(layers, callback) {
    var err = null;
    if (!Array.isArray(layers))
        err = new Error('layers must be an array');

    for (var i = 0;  i < layers.length; i++) {
        if (/[~`!#$%\^&*+=\[\]\\';,/{}|\\"":<>\?]/g.test(layers[i]))
            err = new Error('Invalid characters in layer ' + layers[i]);
    }
    return callback(err);
}
