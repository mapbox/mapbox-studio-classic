...
style.toXML = function(data, callback) {
    tilelive.load(data.source, function(err, backend) {
        ...
        if (opts.interactivity) {
            var err = style.interactivityvalidate(layers, opts.interactivity);
            if (err) return callback(err);
        }
...
style.interactivityvalidate = function(layers, interactivity) {
    ...
    layers.forEach(function(layer,i) {
        if (layer.id === interactivity.layer) {
            ...
            interactivity.fields.forEach(function(interactivityField, i) {
                ...
                if (noMatch) {
                    matchField = false;
                    err = 'Interactivity layer <strong>' + layer.id + '</strong> doesn\'t contain field <strong>' + interactivityField + '</strong> used in the template.'
                }
            });
    ...
    if (!matchLayer) return new Error('Interactivity layer must be present in sources.');
