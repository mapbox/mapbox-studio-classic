...
var style = function(options, callback) {
    var id = options.id;
    ...
    // Reading.
    if (!data) return style.info(id, function(err, data) {
...
// Read/write the style thumb.
style.thumb = function(opts, callback) {
    callback = callback || function() {};

    if (style.tmpid(opts.id) && !opts.thumbpath)
    ...
    return style({id:opts.id}, function(err, source) {
        if (err) return callback(err);
...
style.toPackage = function(options, callback) {
    ...
    style({id:options.id}, function(err, source) {
        if (err) return callback(err);
        style({id:options.id, data:source.data}, function(err) {
            if (err) return callback(err);
            pack();
        });
    });
