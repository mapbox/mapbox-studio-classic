...
// `keys` can be a compound value, separated by .
//   This allows us to track stats with more depth, for example instead
//   of collecting min/max/ave size of each tile at some zoom, we might want 
//   min/max/ave number of coordinates per feature indexed by layer at each zoom
// By specifying a `setName`, the funcion will group statistics into sets that
//   can later be compared. This is useful for determining how effective tile
//   optimization changes might be.
style.stats = function(id, keys, val, setName) {
    if (!cache[id]) return false;
    if ('number' !== typeof val) setName = val;

    var stats = cache[id].stats = cache[id].stats || {};
    if (setName) var statSet = stats[setName] = stats[setName] || {};

    keys.split('.').forEach(function(key, i, keys) {
        var val = i === keys.length - 1 ? { count: 0 } : {};
        stats = stats[key] = stats[key] || val;
        if (setName) statSet = statSet[key] = statSet[key] || val;
    });
    ...
    if ('number' === typeof val) {
        stats = addSample(stats, val);
        if (setName) statSet = addSample(statSet, val);
    }

    return setName ? statSet : stats;
};
