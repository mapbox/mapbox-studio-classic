    ...
    style.info(data.id, function(err, info) {
        ...
        var mapid = (typeof info._prefs.mapid !== 'undefined' && info._prefs.mapid !== '')
            ...
            : data.oauth.account + '.' + crypto.createHash('md5')
                ...
                .digest('hex')
                .substr(0,8);
        var pckage = path.join(data.cache, 'package-' + mapid + '.tm2z');
        if (!fs.existsSync(pckage) && complete === false) {
        ...
        function startUpload() {
            if (!fs.existsSync(pckage)) return callback(null, new task.Done(data.id, 'upload', '/style?id=' + data.id, 1000));
        ...
        function mapSaved() {
            ...
            style.save(info, function(err) {
                ...
                fs.unlink(pckage, function(err) {
                    if (err) return callback(err);
                    complete = true;
                    return callback(null, new task.Done(data.id, 'upload', '/style?id=' + data.id, 1000));
                });
            });
        }
    });
