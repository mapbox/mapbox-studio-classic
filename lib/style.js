...
style.save = function(data, callback) {
            ...
            for (var i = 0;  i < bookmarks.length; i++) {
                ...
                for (var j = 0; j < required.length; j++) {
                    if (!(required[j] in bookmarks[i])) {
                        return new Error('bookmark ' + required[j] + ' is required');
                    }
                }
                ...
                for (var key in bookmarks[i]) {
                    ...
                    case 'place_name':
                        if (typeof val !== 'string')
                            return new Error('bookmark must have a ' + key);
                        break;
                    case 'zoom':
                        if (typeof val !== 'number' || val < 0 || val > 22 || Math.floor(val) !== val)
                            return new Error('bookmark zoom level must be a number between 0 and 22');
                        break;
                    case 'center':
                        if (!Array.isArray(val) || val.length !== 2)
                            return new Error('bookmark center must be an array in the form [lng,lat]');
                        if (typeof val[0] !== 'number' || val[0] < -180 || val[0] > 180)
                            return new Error('bookmark lon value must be between -180 and 180');
                        if (typeof val[1] !== 'number' || val[1] < -90 || val[1] > 90)
                            return new Error('bookmark lat value must be between -90 and 90');
                        break;
                    }
                }
            }
        };
        var err = validateBookmarks(data.bookmarks);
        if (err) return callback(err);
    }
    ...
    style.toXML(data, function(err, xml) {
        ...
        files.push({
            ...
            data: yaml.dump(data.bookmarks, null, 2)
        });
    ...
    return fs.readFile(project_path, 'utf8', function(err, data) {
        ...
        var readbookmarks = function() {
            fs.readFile(bookmarks_path, 'utf8', function(err, bookmarks) {
                ...
                try { data.bookmarks = yaml.load(bookmarks);}
                catch(err) { callback(err); } 

                // Might be valid yaml and yet not be an array.
                // Error out appropriately.
                if (!(data.bookmarks instanceof Array)) {
                    return callback(new Error('Invalid YAML: ' + bookmarks_path));
                }
