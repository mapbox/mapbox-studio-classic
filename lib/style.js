...
style.save = function(data, callback) {
    ...
    if (data._prefs.bookmarks) {
        ...
        var err = validateBookmarks(data._prefs.bookmarks);
        if (err) return callback(err);
    }
    ...
    style.toXML(data, function(err, xml) {
        ...
        files.push({
            ...
            data: yaml.dump(data._prefs.bookmarks, null, 2)
        });
        delete data._prefs.bookmarks;
    ...
    return fs.readFile(project_path, 'utf8', function(err, data) {
        ...
        function readbookmarks() {
            ...
            fs.readFile(bookmarks_path, 'utf8', function(err, bookmarks) {
                ...
                data._prefs = data._prefs || {};
                try { data._prefs.bookmarks = yaml.load(bookmarks);}
                catch(err) { return callback(err); }
