...
style.refresh = function(data, callback) {
    ...
    if (data.layers) {
        err = style.layervalidate(data.layers);
        if (err) return callback(err);
    }

    // validate bookmarks.
    if (data._bookmarks && data._bookmarks.length) {
        err = gazetteer.validate(data._bookmarks);
        if (err) return callback(err);
    }
...
style.save = function(rawdata, callback) {
    ...
    style.refresh(rawdata, function(err, loaded) {
        ...
        // Bookmarks saved to separate file if any exist.
...
style.toXML = function(data, callback) {
    tilelive.load(data.source, function(err, backend) {
        ...
        if (opts.interactivity && backend.data.vector_layers) {
            err = style.interactivityvalidate(backend.data.vector_layers, opts.interactivity);
            if (err) return callback(err);
        }
...
style.toPackage = function(id, dest, callback) {
    ...
    function pack() {
        var writer = typeof dest === 'string' ? fstream.Writer({ path: dest, type: 'File' }) : dest;
...
style.upload = function(id, callback) {
    ...
    style.info(id, function(err, info) {
        ...
        var mapid;
        try {
            mapid = info._prefs.mapid || tm.mapid();
        } catch(err) {
            return callback(err);
        }
...
style.interactivityvalidate = function(layers, interactivity) {
    var matchLayer = false;
    var matchField = true;
    var err = null;

    layers.forEach(function(layer) {
        if (layer.id === interactivity.layer) {
            matchLayer = true;

            // Iterate through all fields to
            // Make sure all interactivity fields have a match
            var layerFields = Object.keys(layer.fields);

            interactivity.fields.forEach(function(interactivityField) {
                var noMatch = layerFields.every(function(layerField) {
                    return layerField !== interactivityField;
                });
                if (noMatch) {
                    matchField = false;
                    err = 'Interactivity layer <strong>' + interactivity.layer + '</strong> doesn\'t contain field <strong>' + interactivityField + '</strong> used in the template.';
                }
            });
    ...
    if (!matchLayer) return new Error('Interactivity layer <strong>' + interactivity.layer + '</strong> must be present in sources.');
    if (!matchField) return new Error(err);

};

// Hash of ids to tmp example styles.
var machineStyles = require('mapbox-machine-styles');
style.examples = {};
style.examples['mapbox-studio-default-style'] = 'tmpstyle://' + tm.join(path.dirname(require.resolve('mapbox-studio-default-style')));
Object.keys(machineStyles).reduce(function(memo, id) {
    memo[id.replace('mapbox.', 'mapbox-studio-')] = 'tmpstyle://' + tm.join(machineStyles.dirname(id));
    return memo;
}, style.examples);
