...
# darwin: add app resources, zip up
    ...
    aws s3 cp "s3://mapbox/mapbox-studio/keys/Mac Developer ID Application: Mapbox, Inc..pem" signing-key.pem
    ...
    security import signing-key.pem -k ~/Library/Keychains/signing.keychain -T /usr/bin/codesign \
        && echo "+ signing pem added to keychain"
